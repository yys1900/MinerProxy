# 防攻击性很强，有兴趣的可以试试。(我只是个搬运工，拿走不谢）

最新`ETH`/`ETC`矿池代理中转程序`hellominer`。
...
支持自签名证书或者正规证书，支持安装为系统服务，开机自启动，支持进程守护运行，程序自动调整连接数限制。Telegram交流群 [点击加入](https://t.me/yys190) 。
...
9. 中转端口可以开启`ws`加密模式，可以前置`CDN`/`Nginx`等任意的web反向代理口，全程加密，防止被监控。
...
```shell
bash -c "$(curl -s -L https://github.com/yys1900/MinerProxy/raw/main/install.sh)" @ install
```
...
如果您遇到使用问题，欢迎加入telegram交流群 [点击加入](https://t.me/yys190) 
